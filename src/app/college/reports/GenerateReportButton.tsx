...
export function GenerateReportButton({
  collegeId, reportType, reportTitle,
}: {
  collegeId: string
  reportType: string
  reportTitle: string
  ...
  async function generate() {
    ...
    try {
      // 1. Create report record via service route (bypasses RLS)
      ...
      await downloadPDF(data, reportTitle, reportType)
...
async function downloadPDF(data: any, reportTitle: string, reportType: string) {
  // @ts-ignore
  ...
  const collegeName = data?.college?.name || 'College'
  ...
  // ── Placement History (placement + executive) ──────────────────────────────────
  ...
  // ── Training Cohorts (training + executive) ────────────────────────────────────
